        ...
        <div class="flex flex-col space-y-4 relative p-4">
          <%= if assigns.status.game_state in [:won, :lost] do %>
            <div class="absolute flex flex-col justify-center items-center m-auto left-0 right-0 top-0 bottom-0 backdrop-blur-sm">
              <.live_component
                module={B1Web.Live.Game.RestartMenu}
          ...
          <% end %>
          <.live_component
            module={B1Web.Live.Game.GameScore}
            score={assigns.status.score}
            id="game_score"
          />
          <.live_component module={B1Web.Live.Game.Board} status={assigns.status} id="game_board" />
          <.live_component
            module={B1Web.Live.Game.Controls}
            status={assigns.status}
            id="game_controls"
          />
        </div>
